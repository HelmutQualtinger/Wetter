...
# ================================
# 1. Full data: 100 most populous Austrian municipalities (2025 estimates)
# ================================
...
# Optional: sort by population descending
...
print(df.head(10))
print(f"\nDataFrame has {len(df)} rows (top 100 populated towns in Austria)")

# ================================
# 3. Save to CSV
# ================================

csv_filename = "austria_top100_towns_2025.csv"
...
MYSQL_DATABASE = "geodata"      # make sure this DB exists

# Create connection string
engine = create_engine(f"mysql+pymysql://{MYSQL_USER}:{MYSQL_PASSWORD}@{MYSQL_HOST}:{MYSQL_PORT}/{MYSQL_DATABASE}")

# Write to MySQL (table will be created or replaced)
table_name = "austria_top100_towns"
df.to_sql(table_name, con=engine, if_exists='replace', index=False)

print(f"Successfully saved {len(df)} records to MySQL table `{table_name}` in database `{MYSQL_DATABASE}`")
